...
void main() {
  ...
  testWidgets('caps its width on a wide parent', (tester) async {
    await tester.pumpWidget(MaterialApp(
      home: Scaffold(
        body: SizedBox(
          width: 1000,
          child: TwoColumnButtonGrid(children: [
            for (var i = 0; i < 2; i++)
              SizedBox(height: 40, child: Text('wide$i')),
          ]),
        ),
      ),
    ));
    final first = tester.getTopLeft(find.text('wide0'));
    final second = tester.getTopLeft(find.text('wide1'));
